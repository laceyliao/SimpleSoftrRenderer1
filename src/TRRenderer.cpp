...
namespace TinyRenderer
{
	...
	void clip_with_w(const std::vector<TRShaderPipeline::VertexData>& vertices,
		std::vector<TRShaderPipeline::VertexData>& ans) {
		const float w_plane = 1e-5;
		int num = vertices.size(), cur_idx, pre_idx;
		for (int i = 0; i < num; ++i) {
			cur_idx = i;
			pre_idx = (i - 1 + num) % num;
			TRShaderPipeline::VertexData pre_vertex = vertices[pre_idx];
			TRShaderPipeline::VertexData cur_vertex = vertices[cur_idx];
			bool d1 = pre_vertex.cpos.w > w_plane;
			bool d2 = cur_vertex.cpos.w > w_plane;
			if (d1 != d2) {
				float t = (w_plane - pre_vertex.cpos.w) / (pre_vertex.cpos.w - cur_vertex.cpos.w);
				TRShaderPipeline::VertexData tmp = TRShaderPipeline::VertexData::lerp(pre_vertex, cur_vertex, t);
	...
	void clip_with_plane(int plane,const std::vector<TRShaderPipeline::VertexData>& vertices, 
		std::vector<TRShaderPipeline::VertexData>& ans)
	{
		int idx, num=vertices.size(),cur_idx, pre_idx;
		if (plane == 0) {
			idx = 0;
		}
		else if (plane == 1) {
		...
		std::vector<TRShaderPipeline::VertexData> tmp_list;
		for (int i = 0; i < num; ++i) {
			cur_idx = i;
			pre_idx = (i - 1 + num) % num;
			TRShaderPipeline::VertexData pre_vertex = vertices[pre_idx];
			TRShaderPipeline::VertexData cur_vertex = vertices[cur_idx];
			bool d1 = pre_vertex.cpos[idx] < pre_vertex.cpos.w;
			bool d2 = cur_vertex.cpos[idx] < cur_vertex.cpos.w;
			if (d1 != d2) {
				float t = (pre_vertex.cpos.w - pre_vertex.cpos[idx]) / ((pre_vertex.cpos.w - pre_vertex.cpos[idx]) - (cur_vertex.cpos.w  - cur_vertex.cpos[idx]));
				TRShaderPipeline::VertexData tmp = TRShaderPipeline::VertexData::lerp(pre_vertex, cur_vertex, t);
				tmp_list.emplace_back(tmp);
			}
			if (d2) {
				tmp_list.emplace_back(cur_vertex);
			}
		}
		//opposite face
		num = tmp_list.size();
		for (int i = 0; i < num; ++i) {
			cur_idx = i;
			pre_idx = (i - 1 + num) % num;
			TRShaderPipeline::VertexData pre_vertex = tmp_list[pre_idx];
			TRShaderPipeline::VertexData cur_vertex = tmp_list[cur_idx];
			bool d1 = pre_vertex.cpos[idx] > -pre_vertex.cpos.w;
			bool d2 = cur_vertex.cpos[idx] > -cur_vertex.cpos.w;
			if (d1 != d2) {
				float t = (pre_vertex.cpos.w + pre_vertex.cpos[idx]) / ((pre_vertex.cpos.w + pre_vertex.cpos[idx]) - (cur_vertex.cpos.w + cur_vertex.cpos[idx]));
		...
		const TRShaderPipeline::VertexData &v2) const
	{
		...
		//float near = m_frustum_near_far.x, far = m_frustum_near_far.y;
		...
		clip_with_w({ v0,v1,v2 },in_list);
		clip_with_plane(0, in_list, in_list1);
		clip_with_plane(1, in_list1, in_list2);
		clip_with_plane(2, in_list2, in_list3);
	...
	bool TRRenderer::isTowardBackFace(const glm::vec4 &v0, const glm::vec4 &v1, const glm::vec4 &v2) const
	{
		...
		glm::vec3 n = glm::normalize(glm::cross(tmp1, tmp2));
		glm::vec3 eye = glm::vec3(0, 0, -1);
		return glm::dot(n, eye) > 0;
		//return false;
	}
